    ...
    <section id="diagnostics" ref={ref} style={{ background: "var(--color-bg)", padding: "15rem 2rem", borderBottom: "1px solid var(--color-border)" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} style={{ marginBottom: "8rem" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(3.5rem, 6vw, 4.5rem)", fontWeight: 300, color: "#ffffff" }}>Refinement & Validity.</h2>
        </motion.div>
                ...
                <tbody>
                  {violatingVars.map((v, i) => (
                    <tr key={i} style={{ borderBottom: i < 2 ? "1px solid var(--color-border)" : "none" }}>
                      <td style={{ padding: "1.5rem", fontFamily: "var(--font-mono)", fontSize: "0.9rem", color: "#ffffff", fontWeight: 600 }}>{v.variable}</td>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {aftDistributions.map((d, i) => (
                    <tr key={i} style={{ borderBottom: i < 3 ? "1px solid var(--color-border)" : "none", background: d.selected ? "rgba(0,51,153,0.1)" : "transparent" }}>
                      <td style={{ padding: "1.5rem", fontFamily: "var(--font-mono)", fontSize: "0.9rem", color: d.selected ? "var(--color-accent)" : "#ffffff", fontWeight: d.selected ? 800 : 400 }}>
                        {d.dist}
                        {d.selected && <span style={{ marginLeft: "1rem", fontSize: "0.6rem", background: "var(--color-accent)", color: "#000", padding: "0.3rem 0.6rem", borderRadius: "2px", fontWeight: 900 }}>SELECTED</span>}
                      </td>
